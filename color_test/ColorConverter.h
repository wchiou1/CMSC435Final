...
//functions prototype 
void RGB_to_XYZ(const float RGB[3], float result[3]);
void RGB_to_LAB(const float RGB[3], float result[3]);
void XYZ_to_RGB(const float XYZ[3], float result[3]);
void XYZ_to_LAB(const float XYZ[3], float result[3]);
void LAB_to_RGB(const float LAB[3], float result[3]);
void LAB_to_XYZ(const float LAB[3], float result[3]);
void multiply(const float m[3][3],const float* v, float result[3]);

//pow(x,y) returns nan when x=0? make it return 0 instead.
float pow_(const float x,const float y){ 
